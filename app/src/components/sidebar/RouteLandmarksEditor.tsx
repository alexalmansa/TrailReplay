...
export function RouteLandmarksEditor() {
  ...
  return <div className="space-y-3 rounded-lg border border-[var(--evergreen)]/15 bg-[var(--evergreen)]/3 p-3">
    ...
    {nearbyPlacesEnabled && !nearbyPlacesLoading && !nearbyPlacesError && <p className="text-xs text-[var(--evergreen-60)]">{enrichedLandmarks.length > 0 ? `${enrichedLandmarks.length} named places loaded${nearbyPlacesCoverage?.source === 'landmark-database' ? ' from the TrailReplay landmark database' : ' from OpenStreetMap'}.` : 'Nearby places load when you open a route.'}</p>}
